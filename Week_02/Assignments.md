...
**并发收集器(G1)**,多核，JVM占用较大内存的应用；想要更可控，可预期的GC停顿周期；预期在运行过程中会产生大量内存碎片。(实际测试中体现当堆内存较大时，垃圾收集策略相对简单，当堆内存较小时，收集策略相对较复杂。)

- 能充分利用多cpu多核环境，并行缩短STW时间，并发不暂停用户线程执行降低延迟
- 整堆收集，物理上不在划分年轻代和老年代，划分多个大小相等的Region
- 结合多种垃圾收集算法，空间整合不产生碎片，有利于长时间运行(不同的Region之间通过复制算法)
- 可预期停顿(会根据优先级，可以选择价值最大Region先回收)
